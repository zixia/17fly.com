...
/*
	...
	$Id: stats.php 16688 2008-11-14 06:41:07Z cnteacher $
*/
...
if(empty($type)) {
	...
	for($i = 0; $i < $statvars['forums']; $i++) {
		$bgclass = $i % 2 ? ' class="colplural"' : '';
		@$forumsrank .= "<tr".$bgclass."><td class=\"stat_subject\"><a href=\"forumdisplay.php?fid={$threads[$i]['fid']}\" target=\"_blank\">{$threads[$i]['name']}</a></td><td class=\"stat_num\">{$threads[$i]['threads']}</td>\n".
			...
			"<td class=\"stat_subject\"><a href=\"forumdisplay.php?fid={$thismonth[$i]['fid']}\" target=\"_blank\">{$thismonth[$i]['name']}</a></td><td class=\"stat_num\">{$thismonth[$i]['posts']}</td>\n".
	...
	foreach($arrextcredits as $k=>$extcredits) {
		for($i = 0; $i < 20; $i++) {
			...
			$creditsrank[$k] .= $k == 0 ? "<td class=\"stat_subject\"><a href=\"space.php?username=".rawurlencode($credits[$i]['username'])."\" target=\"_blank\">{$credits[$i]['username']}</a>&nbsp;</td><td class=\"stat_num\">{$credits[$i]['credits']}</td>\n" : '';
			foreach($extcredits as $id => $credit) {
				@$creditsrank[$k] .= "<td class=\"stat_subject\"><a href=\"space.php?username=".rawurlencode($extendedcredits[$id][$i]['username'])."\" target=\"_blank\">{$extendedcredits[$id][$i]['username']}</a></td>
				<td class=\"stat_num\">{$extendedcredits[$id][$i]['credits']} $credit[unit]&nbsp;</td>\n";
			}
			$creditsrank[$k] .= "</tr>\n";
		}
	}
