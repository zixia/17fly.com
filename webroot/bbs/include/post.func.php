...
/*
	...
	$Id: post.func.php 17177 2008-12-09 05:09:46Z tiger $
*/
...
function attach_upload($varname = 'attach', $swfupload = 0) {
	...
	foreach($attachments as $key => $attach) {
		...
		if($attach_saved) {
			...
			$attach['width'] = $attach['isimage'] ? $width : 0;
...
function checksmilies($message, $smileyoff) {
	...
	if($smileyoff) {
		...
		if(!empty($_DCACHE['smilies_display']) && is_array($_DCACHE['smilies_display'])) {
			$message = stripslashes($message);
			foreach($_DCACHE['smilies_display'] AS $stypeid => $smilies) {
