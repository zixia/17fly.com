...
/*
	...
	$Id: cache.func.php 17303 2008-12-12 05:38:18Z monkey $
*/

define('DISCUZ_KERNEL_VERSION', '7.0.0');
define('DISCUZ_KERNEL_RELEASE', '20081212');
...
function updatecache($cachename = '') {
	...
	static $cachescript = array
		...
		'viewthread'	=> array('smilies', 'smileytypes', 'smilies_display', 'forums', 'usergroups', 'ranks', 'bbcodes', 'smilies', 'advs_viewthread', 'tags_viewthread', 'custominfo', 'groupicon'),
	...
	if(!$cachename || $cachename == 'styles') {
		...
		while($data = $db->fetch_array($query)) {
			...
			$data['imagemaxwidth'] = intval($data['contentwidth']) <= 100 ? 600 : intval($data['contentwidth']);
	...
	switch($cachename) {
		case 'settings':
			...
			while($nav = $db->fetch_array($query)) {
				...
				if($db->result_first("SELECT COUNT(*) FROM {$tablepre}navs WHERE parentid='$nav[id]' AND available='1'")) {
					...
					if($nav['id'] == '3') {
						...
						$purl = parse_url(str_replace('&amp;', '&', $nav['url']));
						$getvars = array();
						if($purl['query']) {
			...
			if(@include(DISCUZ_ROOT.'./uc_client/data/cache/apps.php') && is_array($_CACHE['apps'])) {
				$data['allowsynlogin'] = $_CACHE['apps'][UC_APPID]['synlogin'] && count($_CACHE['apps']) > 1;
			...
			$data['homeshow'] = $data['uchomeurl'] && $data['uc']['homeshow'] ? $data['uc']['homeshow'] : '0';
/*
			if($data['uchomeurl']) {
