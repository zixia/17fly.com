...
/*
	...
	$Id: uchome.php 17188 2008-12-09 07:18:47Z liuqiang $
*/
...
if($action == 'getalbums' && $discuz_uid) {
	...
	if($photoes && is_array($photoes)) {
		...
		foreach($photoes as $photo) {
			...
			$str .= '<td><img src="'.$uchomeurl.'/'.$photo['pic'].'" title="'.$photo['filename'].'" onclick="wysiwyg ? insertText(\'<img src='.$uchomeurl.'/'.$photo[bigpic].' border=0 /> \', false) : insertText(\'[img]'.$uchomeurl.'/'.$photo[bigpic].'[/img]\');hideMenu();" onload="thumbImg(this, 1)" _width="80" _height="80" style="cursor: pointer;"></td>'.
