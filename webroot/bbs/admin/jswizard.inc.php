	...
	if(!isset($sideid)) {
		...
		while($request = $db->fetch_array($query)) {
			if($request['type'] == -2) {
				...
				$sides[] = array($request['variable'], $v);
		...
		showsetting('jswizard_infoside_project', array('infosidenew', $sides), $sidekey, 'select', '', 0, '', "onchange=\"if(this.value && this.value != -1) {window.location='$BASESCRIPT?action=jswizard&operation=infoside&sideid=$sideid&from=$from&sidekey='+this.value} else if(this.value == -1) {\$('newproject').style.display = '';} else {\$('newproject').style.display = 'none';}if(this.value) {\$('modulelist').style.display = '';} else {\$('modulelist').style.display = 'none';}\"");
