			...
			if($failedlogin['count'] > 4) {
				...
				if(empty($seccodehidden) || $seccodehidden != $seccode) {
					...
					if($isfounder == 1) {
						...
						$admin = $this->db->fetch_first("SELECT a.uid,m.salt,m.password,m.username FROM ".UC_DBTABLEPRE."admins a LEFT JOIN ".UC_DBTABLEPRE."members m USING(uid) WHERE a.username='$username'");
						if(!empty($admin)) {
