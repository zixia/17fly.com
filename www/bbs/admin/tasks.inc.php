...
/*
	...
	$Id: tasks.inc.php 17067 2008-12-05 01:56:12Z liuqiang $
*/
...
if(!($operation)) {
	...
	if(!submitcheck('addsubmit')) {
		...
		showsetting('tasks_add_usergroup', array('applyperm[]', fetcharray('groupid', 'grouptitle', 'usergroups', "type!='system'")), 0, 'mselect');
		showtagfooter('tbody');
	...
	if(!submitcheck('editsubmit')) {
		...
		showsetting('tasks_add_usergroup', array('applyperm[]', fetcharray('groupid', 'grouptitle', 'usergroups', "type!='system'")), explode("\t", $task['applyperm']), 'mselect');
		showtagfooter('tbody');
		...
		if($task['scriptname'] == 'member') {
			...
			showsetting('tasks_add_limit_threadid', 'threadid', $taskvars['complete']['threadid']['value'], 'text');
...
function fetcharray($id, $name, $table, $conditions = '1') {
	...
	$query = $db->query("SELECT $id, $name FROM $tablepre$table WHERE $conditions");
